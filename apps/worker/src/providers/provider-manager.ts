...
export class ProviderManager {
  ...
  /** Llamar al LLM con fallback automático entre providers */
  async callWithFallback(task: StageTask): Promise<ProviderResult> {
    ...
    for (const provider of ranked) {
      ...
      try {
        ...
        if (!isRetryable) {
          throw err; // Error no recuperable (401, 403, 400)
        }
        console.warn(
  ...
  private rankProviders(stage: string): ProviderConfig[] {
    // Priorizar por costo: más barato primero
